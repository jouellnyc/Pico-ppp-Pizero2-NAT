...
**Wiring:**
```
Pico W GP4 (TX) → Pi Zero GPIO15 (RX) — physical pin 10
Pico W GP5 (RX) → Pi Zero GPIO14 (TX) — physical pin 8
Pico W GND      → Pi Zero GND          — physical pin 6
```
...
Expands ppp0 transmission queue depth on link initialization.

```bash
echo "ip link set \$1 txqueuelen 100" | sudo tee /etc/ppp/ip-up.d/99-txqueuelen
...
PPP support is enabled by default for Pico W in MicroPython master as of June 2026. Build from source or use a recent nightly. 

Build your on with an H (see official docs).
...
- `rxbuf=2048` on the UART is not optional — without it recv() fails ~90%
  of the time due to buffer overflow on inbound HTTP responses
